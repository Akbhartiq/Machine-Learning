...
def plt_intuition(x_train, y_train):
    ...
    w_range = np.array([200-200, 200+200])  # create an array
    tmp_b = 100  # set a temp b value to 100

    w_array = np.arange(*w_range, 5)  # [0,5,10,...,400]
    cost = np.zeros_like(w_array)  # [0,0,...,0]
    for i in range(len(w_array)):
        tmp_w = w_array[i]
        cost[i] = compute_cost(x_train, y_train, tmp_w, tmp_b)

    @interact(w=(*w_range, 10), continuous_update=False)
    def func(w=150):  # w is set to 150
        # Directly calculating the Prediction
        f_wb = np.dot(x_train, w) + tmp_b
        # print(f"Inside the func {x_train} and {w} and {tmp_b}")
